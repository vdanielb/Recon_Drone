...
linux/pipeline.py            Orchestrator: parse packets, log, map, scene labels, fuse detections, status.json
linux/main.py                **UNO Q App Lab entry** — bridge telemetry + mapping + edge YOLO + dashboard
...
linux/dashboard.py           Flask dashboard (board-hosted via main.py, or laptop-only)
...
degrees in [-180, 180] from the MPU6050 gyro; `pipeline.py` sets `mapper.pose.theta`
...
`pipeline.py` writes `data/logs/status.json` on every SCAN (every 7 sweeps or on
...
- **Degrades gracefully**: missing cv2/onnxruntime/camera/model → detection
  disabled, `Detector.available=False`, app continues. The import in `pipeline.py`
  is itself guarded.
...
- Per-category cooldown in `pipeline.py` prevents map-tag spam.
...
## UNO Q deployment (primary — board-hosted dashboard)

On the UNO Q, the MCU does **not** expose CSV telemetry as a Linux serial device
(e.g. `/dev/ttyACM0`). Lines arrive via App Lab **Bridge**
(`Bridge.notify("telemetry", ...)` on the sketch; `Bridge.provide` in Python).
Use `--source bridge` or the bundled entrypoint.

**Board (App Lab main script):** [linux/main.py](linux/main.py) — the App Lab
Python entrypoint. It registers the Bridge, then launches `pipeline.py --source
bridge` (mapping + edge YOLO) and `dashboard.py` in daemon threads. Requires
`arduino.app_utils`, so it only runs on the board (SystemExit elsewhere).

1. Copy `linux/` to the board (including `models/yolov8n_int8.onnx`).
2. Install on the board: `pip install Flask matplotlib opencv-python onnxruntime`
   (no PyTorch/Ultralytics on the board).
3. Set the App Lab Python main to `main.py` (orchestrator lives in `pipeline.py`).
4. Join the laptop Wi-Fi hotspot; note the board IP (e.g. `192.168.137.x` via
   `ip addr` on the board).
5. On the laptop browser: `http://<board-ip>:8000` — map, radar, events, and
   camera feed all come from the board's `data/logs/`.

Environment overrides: `DARKMAP_CAMERA` (default `0`), `DARKMAP_MODEL`
(default `models/yolov8n_int8.onnx`), `DARKMAP_DASHBOARD_HOST` (default
`0.0.0.0`), `DARKMAP_DASHBOARD_PORT` (default `8000`).

**Legacy laptop-hosted pipeline:** `uno_q_forwarder.py` on the board +
`python3 pipeline.py --source net` on the laptop. Dashboard/camera on the laptop
only see data if `pipeline.py` runs there (no rover camera over TCP).

Manual pipeline on the board (without `main.py`):

```bash
cd linux
python3 pipeline.py --source bridge --no-plot --model models/yolov8n_int8.onnx
python3 dashboard.py --host 0.0.0.0 --port 8000   # second terminal / thread
```
...
**Primary:** served by `main.py` on the UNO Q at `http://<board-ip>:8000`.
**Dev/sim:** run standalone on a laptop at `http://127.0.0.1:8000` while
`pipeline.py` writes `data/logs/status.json` locally.
...
```bash
cd linux
python3 pipeline.py --source sim --no-plot           # headless
python3 pipeline.py --source sim                     # live map window
python3 pipeline.py --source sim --sim-steps 300     # longer run
```
...
python3 pipeline.py --source wallsim --sim-steps 300 --hold

# Rectangle
python3 pipeline.py --source wallsim --room rect:500x300 --sim-steps 300 --hold

# Circle (diameter in cm, optional segment count)
python3 pipeline.py --source wallsim --room circle:300 --sim-steps 450 --hold
python3 pipeline.py --source wallsim --room circle:300:64 --sim-steps 500 --hold

# Equilateral triangle (side length in cm)
python3 pipeline.py --source wallsim --room triangle:300 --sim-steps 350 --hold

# L-shaped room (tests open outer corner)
python3 pipeline.py --source wallsim --room l-shape --sim-steps 400 --hold

# Custom polygon (comma-separated x,y vertex pairs in cm)
python3 pipeline.py --source wallsim --room "poly:0,0,400,0,400,300,0,300" --sim-steps 400 --hold

# Headless (no display)
python3 pipeline.py --source wallsim --room square:300 --sim-steps 300 --no-plot
```
